...
static volatile unsigned int c_baud = 0;
static volatile unsigned int c_total_bits = 0;
static volatile unsigned int c_ms_char = 0;
static volatile unsigned int c_us_char = 0;

//static unsigned int linespeeds[] = {0, 75, 110, 150, 300, 600, 1200, 2400, 4800, 9600, 14400, 19200, 38400, 57600, 115200};
static unsigned int linespeeds[] = {0, 4800, 9600, 14400, 19200, 38400, 57600, 115200};
...
//const uint32_t baud[]   = {300, 600, 1200, 2400, 4800, 9600, 14400, 19200, 28800, 38400, 57600, 115200, 0};
const uint32_t baud[]   = {4800, 9600, 14400, 19200, 38400, 57600, 115200, 0};
...
static unsigned long nextChar = 0;        // limit data rate
                      ...
                      if( cmd[ptr]=='?' )
                        {
                          ...
                          Serial.print(",");
                          Serial.print(SerialData.baud);
                          Serial.print(",");
                          Serial.print(SerialData.bits);
                          Serial.print(",");
                          Serial.print(SerialData.stopbits);
                          printModemCR();
                          
                          Serial.print(modemReg[REG_CURLINESPEED]);
                          Serial.print(",");
                          Serial.print(linespeeds[modemReg[REG_CURLINESPEED]]);
                          printModemCR();

                          Serial.print(c_us_char);
                          printModemCR();
                        }
                      else if( cmd[ptr]=='=' )
                        {
...
void updateBPSDelay(){
  c_baud = SerialData.baud;
  c_total_bits = (1 + SerialData.bits + SerialData.stopbits);
  //c_total_bits = c_total_bits + 5;
  float ratio = ((float)c_total_bits / (float)c_baud);
  c_ms_char = ratio * 1000;
  c_us_char = ratio * 1000 * 1000;
}
...
void delayBPS(){
    ...
    //if(c_us_char >=100 && c_us_char <=999)
      delayMicroseconds(c_us_char);
...
/*
 ...
 for (i = 0; i < MAX_SRV_CLIENTS; i++) 
        if( serverClients[i] && serverClients[i].connected() ){
                ...
                serverClients[i].write(itoa((int)c_us_char, buf, 10));
